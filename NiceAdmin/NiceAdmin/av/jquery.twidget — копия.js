...
;( function( $, window, document, undefined ) {
    ...
    $.extend( Plugin.prototype, {
        ...
        widget_html: function() {
            ...
            '                <form id="myform" action="' + _this.settings.localization.avia_submit_domain + '" method="get" autocomplete="off" ' + (_this.settings.open_in_new_tab ? 'target="_blank"' : '') + '>'+
            ...
            '                            <button class="submit_button" type="button">'+_this.settings.localization.avia_submit_button_text+'</button>'+
            '                        </li>'+
            '                    </ul>'+
            '                </form>'+
    ...
    $(document).on('click', '.submit_button', function () {
        var serializeData = $('#myform').serialize();
        $.ajax({
            url: "searchtickets?"+serializeData,
            dataType: 'json',
            async: true,
            beforeSend: function () {
            ...
            success: function(data) {
                ...
                if(resp.success==true){
                    ...
                    var tbl = "<table width = '95%' style='textAlign:center;' border='2' class='minimalistBlack'>";

                    var captions=["Рейс", "Откуда","Отправление","Куда"
                    ,"Прибытие","В пути","Пересадки","Класс"
                    ,"Тариф","Багаж","Цена","-"];

                    tbl=tbl+"<tr bgcolor='#00CC99'>";
                    for(var i=0;i<captions.length;i++){
                        tbl=tbl+"<td style='text-align:center;'>"+captions[i]+"</td>";
                    }


                    var recommendations=resp.data.recommendations;
                    var outboundFlights=resp.data.outboundFlights;
                    var currency=resp.data.currency;
                    for(var recNo=0;recNo<recommendations.length;recNo++){
                        tbl=tbl+"<tr style='text-align:center;'><form method='GET' id='rec"+recNo+"' name='rec"+recNo+"' >";

                        var outboundFlightGroupId=recommendations[recNo].outboundFlightGroupId;
                        var outboundSeatsLeft=recommendations[recNo].outboundSeatsLeft;
                        var baggageQuantity=0;
                        var baggageWeight=0;
                        var totalPrice=recommendations[recNo].totalPrice;

                        //tarif;
                        var fareBasis=[];//tarif
                        var bookingCode=[];//class
                        var fares=recommendations[recNo].outboundFareAndAmountDetails;
                        for(var farNo=0;farNo<fares.length;fares++){
                            fareBasis.push(fares[farNo].fareBasis);
                            bookingCode.push(fares[farNo].bookingCode);
                        }
                        //tarif end;

                        //reys
                        var operatingCarrier=outboundFlights[outboundFlightGroupId].flights[0].operatingCarrier;
                        var flightNumber=outboundFlights[outboundFlightGroupId].flights[0].flightNumber;
                        var Flight=operatingCarrier+" "+flightNumber;

                        var operatingCarrierArr=[];
                        var flightNumberArr=[];
                        var departureDateTimeArr=[];
                        var arrivalDateTimeArr=[];
                        var flightDurationArr=[];
                        var operatingCarriers=outboundFlights[outboundFlightGroupId].flights;
                        for(var operNo=0;operNo<operatingCarriers.length;operNo++){
                            operatingCarrierArr.push(operatingCarriers[operNo].operatingCarrier);
                            flightNumberArr.push(operatingCarriers[operNo].flightNumber);

                            departureDateTimeArr.push(operatingCarriers[operNo].departureDateTime);
                            arrivalDateTimeArr.push(operatingCarriers[operNo].arrivalDateTime);
                            flightDurationArr.push(operatingCarriers[operNo].flightDuration);
                        }
                        //пересадка
                        var transfer=operatingCarriers.length;
                        if(recommendations[recNo].outboundBaggageAllowances[0]){
                            baggageQuantity=recommendations[recNo].outboundBaggageAllowances[0].quantity;
                            baggageWeight=recommendations[recNo].outboundBaggageAllowances[0].weight;
                        }

                        var outboundFlightGroupHash=recommendations[recNo].outboundFlightGroupHash;
                        var outboundFlightArr=[];
                        if(transfer>1){
                            const outboundFlight = outboundFlightGroupHash.split(",");
                            for(var i=0;i<outboundFlight.length;i++){
                                var outbound=outboundFlight[i].split("-");
                                outboundFlightArr.push(outbound[outbound.length-2]);
                                outboundFlightArr.push(outbound[outbound.length-1]);
                            }
                        } else {
                            var outbound=outboundFlightGroupHash.split("-");
                            outboundFlightArr.push(outbound[outbound.length-2]);
                            outboundFlightArr.push(outbound[outbound.length-1]);
                        }

                        tbl=tbl+"<td>"+Flight+"</td>";
                        tbl=tbl+"<td>"+outboundFlightArr[0]+"</td>";

                        moment.locale('ru');
                        var dateStr = moment(departureDateTimeArr[0]).format('DD MMM HH:mm');

                        tbl=tbl+"<td>"+dateStr+"</td>";
                        tbl=tbl+"<td>"+outboundFlightArr[outboundFlightArr.length-1]+"</td>";

                        var dateStr = moment(arrivalDateTimeArr[arrivalDateTimeArr.length-1]).format('DD MMM HH:mm');
                        tbl=tbl+"<td>"+dateStr+"</td>";
                        tbl=tbl+"<td>"+flightDurationArr.join("-")+"</td>";
                        tbl=tbl+"<td>"+(transfer+"x")+"</td>";
                        tbl=tbl+"<td>"+(bookingCode[0]+" ("+outboundSeatsLeft+")")+"</td>";
                        tbl=tbl+"<td>"+(fareBasis[0])+"</td>";
                        tbl=tbl+"<td>"+(baggageWeight+"kg")+"</td>";
                        tbl=tbl+"<td>"+(totalPrice+" "+currency)+"</td>";
                        tbl=tbl+"<td><input type='button' id="+recNo+" onclick='eeee(this)'; value='Бронировать'></form></td></tr>";
                    }
                    document.getElementById("recommendations").innerHTML="<br>";
                    document.getElementById("recommendations").innerHTML=tbl;
                    //document.getElementById("recommendations").innerHTML="<center>"+document.getElementById("recommendations").innerHTML+"</center><br><br><br>";
                } else {
                    document.getElementById("recommendations").innerHTML="<center><br><br>"+resp.errors[0].description+"</center>";
                }
            }
          });
    });
