...
;( function( $, window, document, undefined ) {
    ...
    $.extend( Plugin.prototype, {
        init: function() {
            ...
            /* or getting user origin city from whereami */
                ...
                //$.getJSON("https://autocomplete.travelpayouts.com/jravia?locale=ru&with_countries=false&q=%D0%94%D0%BE%D0%BC%D0%BE%D0%B4%D0%B5%D0%B4%D0%BE%D0%B2%D0%BE", function (data) {
                    /* input focusout update */
                    if(data){
                        container.find("#twidget-origin").off('focusout').on('focusout', function(){
                            container.find('#twidget-origin').val(data[0].city_name);
        ...
        widget_html: function() {
            ...
            ' <div class="col"><input type="date" class="form-control" /></div>'+
            ' <div class="col"><input type="text" class="form-control" /></div>'+
            ' <div class="col"><button type="button" class="form-control">Click Me!</button></div>'+
            ' </div>  ');
            */
    ...
    $(document).on('click', '.submit_button', function () {
        ...
        $.ajax({
            url: "apiserena/searchtickets.php?"+serializeData,
            async: true,
            beforeSend: function () {
